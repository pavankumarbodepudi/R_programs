---
title: "Week 4 Assignment: Pavan Kumar Bodepudi"
format: html
editor: visual
---
...
```{r, echo=TRUE}
# Insert your R code for Question 1 in here
# HINT: Check the practice materials to see how one can use which() function along with names () function to extract the coefficients. 

# Your code should print out the list variable names
...
data.frame(MSPE = mean(model_full$residuals^2), ADJR2 = summary(model_full)$adj.r.squared, AIC = AIC(model_full, k = 2), BIC =AIC(model_full, k = log(length(trainData))))
...
model_full_kfold_5 <- cv.glm(data = trainData, glmfit = modglm, K=5)
model_full_kfold_5$delta[1]
model_full_kfold_10 <- cv.glm(data = trainData, glmfit = modglm, K=10)
model_full_kfold_10$delta[1]
loocv.model_full = cv.glm(data = trainData, glmfit = modglm, K=nrow(trainData))
loocv.model_full$delta[1]
...
# After creating the leaps object, tou can first fet the index of best model with which () function, then print the best AdjR2 values for each set
...
adjr2.1 = leaps(trainData[,-17], trainData$logSalary, method = "adjr2", nbest = 1, names = col_names)
cbind(as.matrix(adjr2.1$which))
names(trainData)
col_names = names(trainData$logSalary)[-17]
columnnames<-names(trainData$logSalary)[-17]
bestsubsets_fullR2<-leaps(trainData[,-17], trainData$logSalary, method = "Cp", nbest=1, names = columnnames)
table_bestsubset<-cbind(as.matrix(bestsubsets_fullR2$which),bestsubsets_fullR2$Cp)
bestmodelCp = which(bestsubsets_fullR2$Cp==min(bestsubsets_fullR2$Cp))
best.Cp = cbind(as.matrix(bestsubsets_fullR2$which),bestsubsets_fullR2$Cp)[bestmodelCp,]
best.Cp


model_bestsub<-lm(logSalary~nAtBat + nHits +  nBB + YrMajor + CrRuns + CrBB + nOuts ,data=trainData)
(mean(residuals(model_bestsub)^2))

AIC(model_bestsub,k=2)
AIC(model_bestsub,k=log(nrow(trainData)))
...
set.seed(5410)
Ridgecv = cv.glmnet(as.matrix(predictors.train), y.train, family='gaussian', type.measure="mse", alpha=0, nfolds=10)
plot(Ridgecv)
Ridgecv$lamda
Ridgecv$cvm
min(Ridgecv$cvm)
...
set.seed(5410)
lassocv = cv.glmnet(as.matrix(predictors.train), y.train, family='gaussian', type.measure="mse", alpha=1, nflods=10)
plot(lassocv)
lassocv$lambda
lassocv$cvm
min(lassocv$cvm)
...
```{r, echo=TRUE}
#  Insert your R code for Question 7 in here
model_full_pred <- predict(model_full, newdata = testData)
model_full_mspe <- mean((model_full_pred - testData$logSalary)^2)
model_full_mspe
model_bestsub_pred <- predict(model_bestsub, newdata = testData)
model_bestsub_mspe <- mean((model_bestsub_pred - testData$logSalary)^2)
model_bestsub_mspe
lambda_ridge <- Ridgecv$lambda.min
ridge_pred <- predict(Ridgecv, s = lambda_ridge, newx =as.matrix(predictors.test))
mean((ridge_pred - testData$logSalary)^2)
lambda_lasso <- lassocv$lambda.min
lasso_pred <- predict(lassocv, s = lambda_lasso, newx =as.matrix(predictors.test))
mean((lasso_pred - testData$logSalary)^2)
